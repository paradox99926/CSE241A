...
# get the max capacitance it can load
...
# * get the largest slack to the current cell
...
proc PtCellDelay { cell_name } {
    ...
    foreach_in_collection each_pin $pin {
        ...
        if {$pinDirection == "in" } {
            ...
            # calculate the delay of that cell
...
# get larger cells, if 80 then return the same cell
...
# * if smallest return "skip"
    if { [regexp {[a-z][a-z][0-9][0-9][smf]01} $cell]} {
	return "skip"
    }
}

# get the next slower vth cell, if already fast stay the same. 
...
proc getNextVtUp { libcellName } {
    # get the next faster vth cell, if already fast stay the same. 
	if { [regexp {[a-z][a-z][0-9][0-9]m[0-9][0-9]} $libcellName] } { 
