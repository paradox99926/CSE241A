...
proc SF { cellName change } {
    ...
    # set mySensitivity [expr {$deltaLeakage * $mySlack / $numPath}]
...
proc GTR { design alpha gamma } {
    ...
    while {!$S(feasible) && $S(leakage) < $best_seen_leakage} {
        ...
        while {$counter < $num_to_commit && [llength $modify_cell] > 0} {
            # Find modification with max sensitivity
            set maxIndex 0
