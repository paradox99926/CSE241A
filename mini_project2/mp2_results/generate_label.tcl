...
set fp [open "training.label.csv" "w"]
puts $fp "netName,util,cp,numVias"

foreach cp $cpList {
  foreach util $utilList {
    puts "Processing design with Utilization: $util%, Clock Period: ${cp}ps"

    set libdir            "../libdir"
    set netlist           "../design/${design}_${util}_${cp}_route.v"
    ...
    defIn ../design/${design}_${util}_${cp}_route.def
    ...
    foreach net [dbGet top.nets] {
      ...
      puts $fp "[dbGet $net.name],$util,$cp,[llength [dbGet -e $net.vias]]"
    }
    freeDesign
  }
}
